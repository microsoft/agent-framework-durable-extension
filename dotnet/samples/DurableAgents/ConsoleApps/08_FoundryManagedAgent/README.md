...
This versioned `FoundryAgent` path does **not** enable `ChatClientAgentOptions.RequirePerServiceCallChatHistoryPersistence`, so the sample leaves `DurableAgentHistoryOptions.ServiceManagedPerServiceCallHistory` disabled during registration. That option is only needed when a service-backed `ChatClientAgent` explicitly enables per-service-call persistence; it is otherwise ignored.
