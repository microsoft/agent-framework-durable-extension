...
// This tool runs the migrated Durable Task samples and verifies their output.
...
//   dotnet run -- DurableWorkflows_Console_01_SequentialWorkflow # Run a specific sample by name
//   dotnet run -- --category durable-agents-console              # Run console durable agent samples
//   dotnet run -- --category durable-workflows-console           # Run console durable workflow samples
...
if (!File.Exists(Path.Combine(dotnetRoot, "agent-framework-durable-extension.slnx")))
{
