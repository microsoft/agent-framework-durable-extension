...
- Fixed agents registered through `ConfigureDurableOptions` generating no functions at all, leaving the agent silently unreachable. Agents now get the same entry points regardless of which configuration method registers them, while agents that exist only because a workflow references them continue to get no HTTP endpoint of their own ([#67](https://github.com/microsoft/agent-framework-durable-extension/pull/67))
