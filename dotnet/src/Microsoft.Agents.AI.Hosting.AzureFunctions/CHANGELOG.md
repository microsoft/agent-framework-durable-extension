...
- Prefer camelCase names on agent HTTP requests and responses (`sessionId`, `waitForResponse`) while continuing to accept legacy snake_case request names during the migration window ([#51](https://github.com/microsoft/agent-framework-durable-extension/issues/51))
