...
from _retention_test_support import JsonStateProvider, RecordingChatClient, _ingestion_messages
...
async def test_media_payloads_survive_policy_matrix_json_reload_and_next_model_call(
    ...
    expected = [
        deepcopy(message.to_dict()) for message in retained if not message.additional_properties.get("_excluded")
    ]
    for payload in expected:
        # Reasoning stays in storage but is not replayed to the model. All other
        # content fields and metadata remain part of the exact payload comparison.
        payload["contents"] = [
