...
from agent_framework_durabletask import AgentEntity, AgentEntityStateProviderMixin, _entities
...
class TestFollowCompactionRetention:
    ...
    @pytest.mark.parametrize("kind", ["injected", "in-memory", "durable"])
    def test_inherited_policy_can_change_when_preparing_an_existing_agent(self, kind: str) -> None:
        providers: list[Any] = []
        if kind == "in-memory":
            providers.append(InMemoryHistoryProvider("chosen"))
        elif kind == "durable":
            providers.append(DurableHistoryProvider("chosen", store_context_from={"selected"}))
        original = _agent(context_providers=providers)
        prepared = ensure_durable_history(original, prune_excluded=False)
        first = _history_providers(prepared)[0]
        assert first.prune_excluded is False and first._prune_excluded_explicit is False
        assert ensure_durable_history(prepared, prune_excluded=False) is prepared

        enabled = ensure_durable_history(prepared, prune_excluded=True)
        second = _history_providers(enabled)[0]
        assert second is not first
        assert second.prune_excluded is True and second._prune_excluded_explicit is False
        assert first.prune_excluded is False
        disabled = ensure_durable_history(enabled, prune_excluded=False)
        assert _history_providers(disabled)[0].prune_excluded is False
        assert second.prune_excluded is True
        assert original.context_providers == providers
        if kind == "durable":
            assert providers[0].prune_excluded is None
            assert second.store_context_from == first.store_context_from == {"selected"}
            assert second.store_context_from is not first.store_context_from

    @pytest.mark.parametrize("explicit", [False, True])
    def test_explicit_policy_stays_pinned_across_repreparation(self, explicit: bool) -> None:
        history = DurableHistoryProvider(prune_excluded=explicit)
        agent = _agent(context_providers=[history])
        for inherited in (False, True, False):
            assert ensure_durable_history(agent, prune_excluded=inherited) is agent
            assert history.prune_excluded is explicit
            assert history._prune_excluded_explicit is True
