...
async def test_implicit_history_matches_core_first_turn_after_strategy(per_call: bool) -> None:
    ...
    assert prepared.context_providers[0].__wrapped__ is compaction
...
async def test_eager_pruning_holds_excluded_pending_tool_groups(resolved: bool) -> None:
    provider = _InMemoryStateProvider()
    call = Message(
        "assistant", [Content.from_function_call(call_id="pending", name="lookup", arguments="{}")], message_id="call"
    )
    provider.state.data.conversation_history.append(
        DurableAgentStateResponse("old", OLD, [DurableAgentStateMessage.from_chat_message(call)])
    )
    old_ids = {"call"}
    if resolved:
        result = Message("tool", [Content.from_function_result(call_id="pending", result="done")], message_id="result")
        provider.state.data.conversation_history.append(
            DurableAgentStateRequest("old", OLD, [DurableAgentStateMessage.from_chat_message(result)])
        )
        old_ids.add("result")
    provider.state.data.conversation_history.append(
        DurableAgentStateRequest("newest", OLD, [stored("newest", "retained")])
    )
    history = DurableHistoryProvider(prune_excluded=True)
    state: dict[str, Any] = {}
    with bound(provider):
        await history.get_messages("session", state=state)
        state[WORKING_BUFFER_KEY][:] = [m for m in state[WORKING_BUFFER_KEY] if m.message_id not in old_ids]
        history.flush(state)
        assert old_ids & set(ids(provider)) == (set() if resolved else old_ids)
        assert (provider.state.data.truncation or {}).get("evictedMessageCount", 0) == (2 if resolved else 0)
        snapshot = deepcopy(provider.state.to_dict())
        history.flush(state)
        assert provider.state.to_dict() == snapshot


@pytest.mark.parametrize("protect_partner", [False, True])
async def test_eager_pruning_preserves_noncontiguous_saved_groups_and_envelope_metadata(
    protect_partner: bool,
) -> None:
    provider = _InMemoryStateProvider()
    first = stored("first", "first")
    partner = stored("partner", "partner", "system" if protect_partner else "user")
    for message in (first, partner):
        message.extension_data = {GROUP_ANNOTATION_KEY: {GROUP_ID_KEY: "persisted-atomic"}}
    entry = DurableAgentStateRequest("old", OLD, [first], extension_data={"envelope": {"keep": [None, False]}})
    entry.unknown_fields = {"futureEnvelope": [0, "雪"]}
    provider.state.data.conversation_history.extend([
        entry,
        DurableAgentStateRequest("unrelated", OLD, [stored("middle", "included")]),
        DurableAgentStateRequest("old", OLD, [partner]),
        DurableAgentStateRequest("newest", OLD, [stored("newest", "retained")]),
    ])
    envelope = deepcopy(entry.to_dict())
    history = DurableHistoryProvider(prune_excluded=True)
    state: dict[str, Any] = {}
    with bound(provider):
        await history.get_messages("session", state=state)
        state[WORKING_BUFFER_KEY][:] = [
            m for m in state[WORKING_BUFFER_KEY] if m.message_id not in {"first", "partner"}
        ]
        history.flush(state)
        assert {"first", "partner"} & set(ids(provider)) == ({"first", "partner"} if protect_partner else set())
        assert any(candidate is entry for candidate in provider.state.data.conversation_history)
        if not protect_partner:
            envelope["messages"] = []
            assert entry.to_dict() == envelope
