...
"""Offline write-admission contracts using only pre-slice production entry points."""
...
def _host(
    ...
    DurableAIAgentWorker(grpc_worker, callback=callback).add_agent(agent)
...
def test_mutable_state_accepts_exact_legacy_versions(version: str) -> None:
    assert DurableAgentState(schema_version=version).to_dict() == _legacy_state(version)
...
def test_empty_object_and_default_remain_fresh_legacy_1_1() -> None:
    assert DurableAgentState.SCHEMA_VERSION == "1.1.0"
    assert DurableAgentState().to_dict() == _legacy_state()
    assert DurableAgentState.from_dict({}).to_dict() == _legacy_state()


@pytest.mark.parametrize("version", REJECTED_VERSIONS)
@pytest.mark.parametrize("entry_point", ["init", "from_dict", "to_dict"])
def test_mutable_state_rejects_shared_future_and_invalid_versions(version: Any, entry_point: str) -> None:
    if entry_point == "init":
        with pytest.raises(ValueError):
            DurableAgentState(schema_version=version)
...
def test_mutable_loader_rejects_malformed_existing_state(raw: Any) -> None:
    snapshot = deepcopy(raw)
    with pytest.raises(ValueError):
        DurableAgentState.from_dict(raw)
    assert raw == snapshot
...
def test_mutable_loader_rejects_every_canonical_v2_shape(kind: str) -> None:
...
@pytest.mark.parametrize("kind", SHARED_KINDS)
def test_mutable_state_getter_rejects_canonical_v2(monkeypatch: pytest.MonkeyPatch, surface: str, kind: str) -> None:
...
async def test_run_rejects_v2_before_parsing_execution_callbacks_or_error_recording(
    monkeypatch: pytest.MonkeyPatch, surface: str, kind: str, request_kind: str, cache_kind: str
) -> None:
...
@pytest.mark.parametrize("cache_kind", ["cold", "loaded-legacy", "altered-legacy"])
@pytest.mark.parametrize("kind", SHARED_KINDS)
def test_writes_check_backing_v2_not_replacement_or_cached_schema(
    monkeypatch: pytest.MonkeyPatch, surface: str, operation: str, cache_kind: str, kind: str
) -> None:
    entity, provider, backing, agent, callback = _host(monkeypatch, surface, _legacy_state(populated=True))
    if cache_kind != "cold":
        cached = entity.state
        if cache_kind == "altered-legacy":
            cached.schema_version = "1.0.0"
    # A genuine legacy cache predates the canonical backing state. Do not relabel v2 as legacy.
    backing.raw = _canonical_state(kind)
    original, snapshot = backing.raw, deepcopy(backing.raw)
    cache = provider._state_cache
    cache_snapshot = cache.to_dict() if cache is not None else None
...
def test_invalid_replacement_does_not_displace_existing_cache_or_write_legacy_backing(
...
@pytest.mark.parametrize("surface", ["plain", "registered"])
@pytest.mark.parametrize("version", ["2.0.0", "3.0.0", "1.1.1"])
def test_persist_rejects_altered_cached_version_even_when_backing_is_legacy(
...
async def test_malformed_backing_is_never_treated_as_fresh_or_overwritten(
    ...
    # A getter may return an already admitted legacy cache. Write paths must still
...
def test_legacy_state_assignment_and_persistence_remain_writable(
    monkeypatch: pytest.MonkeyPatch, surface: str, operation: str
) -> None:
    entity, _, backing, agent, callback = _host(monkeypatch, surface, {})
    candidate = DurableAgentState()
    candidate.data.extension_data = {"legacy": "supported"}
    if operation == "setter":
        entity.state = candidate
    else:
        entity.state.data.extension_data = {"legacy": "supported"}
        entity.persist_state()
...
async def test_legacy_default_writes_success_and_failure_without_v2_protocol(
    monkeypatch: pytest.MonkeyPatch, surface: str, populated: bool, scenario: str
) -> None:
    raw = _legacy_state(populated=True) if populated else {}
    entity, _, backing, agent, callback = _host(monkeypatch, surface, raw, scenario)
    response = (
        await entity.run(_request("new"))
        if surface == "plain"
    ...
    assert backing.set_state.called
    ...
    assert len(history) == (4 if populated else 2)
...
@pytest.mark.parametrize("surface", ["plain", "registered"])
@pytest.mark.parametrize("populated", [False, True])
def test_legacy_reset_uses_real_host_dispatch_and_writes_1_1(
    monkeypatch: pytest.MonkeyPatch, surface: str, populated: bool
) -> None:
    raw = _legacy_state(populated=True) if populated else {}
    entity, provider, backing, agent, callback = _host(monkeypatch, surface, raw)
    entity.reset()
    backing.set_state.assert_called_once_with(_legacy_state())
    assert provider.state.to_dict() == _legacy_state()
    agent.run_calls.assert_not_called()
    callback.on_agent_response.assert_not_called()
    callback.on_streaming_response_update.assert_not_called()
